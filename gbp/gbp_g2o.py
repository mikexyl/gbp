...
class G2OFactorGraph(gbp.FactorGraph):
    ...
    def init_nodes(self):
        self.var_nodes = self.pose_nodes + self.const_nodes
        self.generate_priors_var(weaker_factor=100)
...
def create_g2o_graph(g2o_file, configs):
    ...
    for m in initials.keys():
        new_pose_node = FrameVariableNode(variable_id, 6, m)
        # (x, y, z, qx, qy, qz, qw) -> vector6d
        R = transformations.Quaternion(q=initials[m][3:7]).rot_matrix()
        t = initials[m][:3]
        T = np.eye(4)
        T[:3, :3] = R
        T[:3, 3] = t
        new_pose_node.mu = lie_algebra.se3log(T)
